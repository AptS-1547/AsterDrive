...
pub async fn upsert_record<C: ConnectionTrait>(
    ...
    if let Some(existing) = find_by_blob_id(db, input.blob_id).await? {
        let mut active: blob_media_metadata::ActiveModel = existing.into();
        active.blob_hash = Set(input.blob_hash.to_string());
        active.kind = Set(input.kind);
        active.status = Set(input.status);
        active.metadata_json = Set(input.metadata_json.cloned());
        active.error_message = Set(input.error_message.map(str::to_string));
        active.parser = Set(input.parser.to_string());
        active.parser_version = Set(input.parser_version.to_string());
        active.updated_at = Set(input.now);
        active.update(db).await.map_err(AsterError::from)?;
    } else {
        blob_media_metadata::ActiveModel {
