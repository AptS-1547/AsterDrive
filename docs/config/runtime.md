...
- **链接导入引擎**：`builtin` 使用内置 Rust 下载器；`aria2` 会把实际下载交给管理员维护的 aria2 JSON-RPC 守护进程
- **aria2 RPC 地址 / 密钥 / RPC 超时**：只在引擎为 `aria2` 时使用；RPC 超时是单次 JSON-RPC 调用超时，不替代完整下载超时
...
当引擎为 `aria2` 时，AsterDrive 仍会在派发任务前校验源 URL，但 aria2 守护进程会自行做 DNS 解析和出站连接。运维侧应在网络层隔离 aria2，并把 JSON-RPC 端点限制为只有 AsterDrive 可以访问。
...
| 链接导入引擎、文件大小、速度、并发、请求超时和 aria2 参数 | 之后新建的链接导入任务按新规则判断 |
