...
主控节点在远程节点详情里创建 `local` 接收落点时，只能填相对路径。follower 会把这个相对路径拼到 `managed_ingress_local_root` 下面，避免主控节点直接写宿主机任意目录。
...
接收根目录现在是 `server.follower.managed_ingress_local_root`。

如果旧配置里还写着 `server.managed_ingress_local_root`，启动时会提示迁移到新位置。
:::
