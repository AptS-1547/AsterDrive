...
- `管理 -> 系统设置`：决定登录、WebDAV、回收站、历史版本、审计日志和跨域这类全站行为
- `管理 -> 存储策略`：决定文件真正存到哪里
- `管理 -> 策略组`：决定用户或团队上传时，应该命中哪条存储策略

先把这四层分清，再去改具体选项，基本就不容易改错地方。
...
| 固定 JWT 密钥，或决定首次纯 HTTP 引导怎么处理 | [登录与会话](/config/auth) |
| 改浏览器 Cookie 是否必须走 HTTPS，或改 Token 有效期 | [系统设置](/config/runtime) |
| 改文件真正存放的位置 | [存储策略](/config/storage) |
| 改用户或团队上传走哪条存储路线 | [存储策略与策略组](/config/storage) |
| 改 WebDAV 路径或 WebDAV 上传硬上限 | [WebDAV](/config/webdav) |
| 改回收站、历史版本、团队归档、新用户和新团队默认配额、审计日志、跨域 | [系统设置](/config/runtime) |
