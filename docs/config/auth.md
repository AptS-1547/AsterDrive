...
`config.toml` 里的 `[auth]` 现在只负责认证的静态引导项：
...
真正影响浏览器 Cookie 安全策略和 Token 有效期的值，已经在 `管理 -> 系统设置` 中维护。
...
| `bootstrap_insecure_cookies` | `false` | 首次初始化运行时配置时，是否把浏览器 Cookie 的 HTTPS 要求设为关闭 |
...
- `auth_cookie_secure`、`auth_access_token_ttl_secs`、`auth_refresh_token_ttl_secs` 在 `管理 -> 系统设置` 中维护
