...
| 术语 | 解释 | 相关文档 |
| --- | --- | --- |
| 存储策略 | 定义文件真实落点和上传方式。比如本地目录、S3 / MinIO、远程节点。 | [存储策略](/config/storage) |
...
| 对象存储直传 | 浏览器直接把文件传到 S3 / MinIO，服务端只负责签名和完成确认。 | [上传与大文件](./upload-modes) |
| 服务端转发 | 浏览器先把文件传给 AsterDrive，再由服务端写到 S3 / MinIO。 | [上传与大文件](./upload-modes) |
