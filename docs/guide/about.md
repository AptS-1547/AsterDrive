...
但需要明确边界。**现在的远程节点能力是“远程存储后端”，不是“多主集群”**：
...
- `primary` / `follower` 两种运行模式拆开，避免把所有逻辑耦合到一起
...
硬盘会坏，网络会断，磁盘会满，软件会有 bug。**备份需要由部署方负责**——除非你给数据上 RAID 10，我们也不敢保证你不丢数据。
