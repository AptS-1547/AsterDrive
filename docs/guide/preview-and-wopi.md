...
| 类型 | 适合什么 | 关键要求 |
| --- | --- | --- |
| 内置预览器 | 图片、PDF、文本、视频、ZIP 清单等内置能力覆盖的类型 | AsterDrive 自己提供；部分能力还受系统设置开关限制 |
...
AsterDrive 内置了 ZIP 压缩包预览，但它默认不打开。管理员需要同时确认：
...
- 只支持 ZIP
- 读取 ZIP 中央目录，展示文件夹、文件、大小和修改时间
- 不把压缩包解开到用户文件夹
- 不提供压缩包内单个文件的下载
- 不替代“在线解压”

第一次打开某个 ZIP 时，如果清单还没有缓存，后端会创建 `archive_preview_generate` 后台任务。前端会显示生成中，稍后自动重试；任务完成后，后续打开会直接使用缓存。

ZIP 里的文件名如果显示为乱码，可以在预览工具栏里切换 `文件名编码`。常用选项包括：
...
- 是否允许公开分享页预览
- 源 ZIP 大小、条目数量、manifest 大小和扫描耗时上限

::: warning 分享侧开关要谨慎
分享页 ZIP 预览会把压缩包内部的路径、文件名、大小和修改时间展示给访问者。即使访问者还没下载整个 ZIP，也能看到这些元数据。只在你接受这个行为时开启分享侧预览。
:::
