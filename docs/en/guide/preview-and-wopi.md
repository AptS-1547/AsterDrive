...
| --- | --- | --- |
| Built-in previewer | Images, PDFs, text, videos, ZIP listings, and other types covered by built-in capabilities | Provided by AsterDrive itself; some capabilities are also limited by system setting switches |
...
## Read-Only ZIP Archive Preview

AsterDrive includes ZIP archive preview, but it is disabled by default. Administrators need to confirm both:
...
- ZIP only
- Reads the ZIP central directory and shows folders, files, sizes, and modification times
...
When opening a ZIP for the first time, if the listing is not cached yet, the backend creates an `archive_preview_generate` background task. The frontend shows generation in progress and retries later automatically; after completion, later opens use the cache directly.

If filenames inside the ZIP look garbled, switch `Filename encoding` in the preview toolbar. Common options include:
...
- Whether the global ZIP preview switch is enabled
- Whether logged-in users can preview in personal spaces and team spaces
- Whether public share pages can preview
- Source ZIP size, entry count, manifest size, and scan duration limits

::: warning Enable share-side preview carefully
Share-page ZIP preview exposes internal paths, filenames, sizes, and modification times to visitors. Visitors can see this metadata even before downloading the whole ZIP. Enable share-side preview only when you accept this behavior.
:::

If a ZIP cannot open, check first:
...
- Whether the file is a ZIP, not `rar`, `7z`, or another format
- Whether the source ZIP exceeds the size limit
