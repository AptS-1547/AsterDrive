...
| Term | Meaning | Related docs |
| --- | --- | --- |
| Storage policy | Defines where files actually land and how uploads are performed, such as a local directory, S3 / MinIO, or a follower node. | [Storage Policies](/en/config/storage) |
...
| Direct-to-object-storage upload | The browser uploads directly to S3 / MinIO; the server only signs requests and confirms completion. | [Uploads and Large Files](./upload-modes) |
| Server relay | The browser uploads to AsterDrive first, then the server writes to S3 / MinIO. | [Uploads and Large Files](./upload-modes) |
