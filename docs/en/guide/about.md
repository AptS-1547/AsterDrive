...
Then it needs to route flows. Files do not have to stay on this machine forever. They may land on local disk, go to object storage such as S3 / MinIO, or be written to another AsterDrive node. Browsers, WebDAV clients, Office editors, share pages, and background tasks can all be entry points. AsterDrive's job is to make these entry points, destinations, permissions, and lifecycles explicit and understandable.
...
Today AsterDrive can start from default SQLite + local storage, and it can also connect to PostgreSQL / MySQL, S3-compatible object storage, and remote follower nodes. It can handle regular direct uploads, resumable chunked uploads, S3 presigned uploads, and S3 multipart uploads. It no longer sounds like the original file browser, but it still grew from those same three things: upload, download, and trash.
